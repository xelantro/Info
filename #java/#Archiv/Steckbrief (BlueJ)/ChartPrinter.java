...
public class ChartPrinter extends JFrame {
	...
	public ChartPrinter(File file) throws Exception {
		...
		ImageIO.write(chart, "jpg", new File(file.getParent()+"/chart.jpg"));
	...
	{
		return ChartFactory.createAreaChart("Sort-time", 
				"Number of elements", 
				"Sort-time in ms", 
				createDataset(), 
	...
	{ 
		...
		r.lines().forEach(l -> temp.add(Integer.parseInt(l)));
		r.close();
		temp.trimToSize();
		...
		{
			data.setValue(temp.get(i), "SortTime", ""+i);
		}
