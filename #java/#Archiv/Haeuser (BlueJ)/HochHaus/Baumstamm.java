...
{
    public Baumstamm(int px, int py, int psk)
    {
        super(px, py, psk); 
    }
    ...
    int pX, int pY, int psk)
    {
        ...
        for(int i = 1; i < 90; i++)
        {
            x[i] = x[i-1] + Math.cos(Math.toRadians(alpha))*Math.PI*psk/180;
            y[i] = y[i-1] - Math.sin(Math.toRadians(alpha))*Math.PI*psk/180;
            x[182-i] = x[0]+3*psk-(x[i]-x[0]);
            y[182-i] = y[i];
            alpha++;
        }
        x[90] = x[89];
